...
# This script installs Feedback Gate globally for Cursor IDE
...
FEEDBACK_GATE_DIR="$CURSOR_EXTENSIONS_DIR/feedback-gate"

log_progress "Creating global installation directory..."
mkdir -p "$FEEDBACK_GATE_DIR"

# Copy MCP server files
log_progress "Copying MCP server files..."
cp "$SCRIPT_DIR/feedback_gate_mcp.py" "$FEEDBACK_GATE_DIR/"
cp "$SCRIPT_DIR/requirements_simple.txt" "$FEEDBACK_GATE_DIR/"

# Create Python virtual environment
log_progress "Creating Python virtual environment..."
cd "$FEEDBACK_GATE_DIR"
...
feedback_gate_dir = '$FEEDBACK_GATE_DIR'
...
if has_existing and os.path.exists(config_file):
    try:
        with open(config_file, 'r') as f:
            ...
            # Remove feedback-gate if it exists (we'll add the new one)
            existing_servers.pop('feedback-gate', None)
    except:
        existing_servers = {}

# Add Feedback Gate configuration
existing_servers['feedback-gate'] = {
    'command': os.path.join(feedback_gate_dir, 'venv/bin/python'),
    'args': [os.path.join(feedback_gate_dir, 'feedback_gate_mcp.py')],
    'env': {
        'PYTHONPATH': feedback_gate_dir,
        'PYTHONUNBUFFERED': '1',
...
if python3 -c "import json; json.load(open('$CURSOR_MCP_FILE'))" >/dev/null 2>&1; then
    ...
    log_step "  - feedback-gate (Feedback Gate)"
else
...
timeout 5s python feedback_gate_mcp.py > "$TEMP_DIR/mcp_test.log" 2>&1 || true
deactivate

if grep -q "Feedback Gate 2.0 server initialized" "$TEMP_DIR/mcp_test.log"; then
...
EXTENSION_FILE="$SCRIPT_DIR/cursor-extension/feedback-gate-2.7.3.vsix"
if [[ -f "$EXTENSION_FILE" ]]; then
    log_progress "Installing Cursor extension..."
    
    # Copy extension to installation directory
    cp "$EXTENSION_FILE" "$FEEDBACK_GATE_DIR/"
    ...
    if [[ "$EXTENSION_INSTALLED" == false ]]; then
        ...
        log_step "4. Select: $FEEDBACK_GATE_DIR/feedback-gate-2.7.3.vsix"
...
log_step "   - MCP Server: $FEEDBACK_GATE_DIR"
log_step "   - MCP Config: $CURSOR_MCP_FILE"
log_step "   - Extension: $FEEDBACK_GATE_DIR/feedback-gate-2.7.3.vsix"
...
log_step "3. Or ask Cursor Agent: 'Use the feedback_gate_chat tool'"
echo ""
...
log_step "   - Logs: tail -f $(python3 -c 'import tempfile; print(tempfile.gettempdir())')/feedback_gate_v2.log"
log_step "   - Test SoX: sox --version"
log_step "   - Browser Console: F12 in Cursor"
echo ""
log_success "Enjoy your voice-activated Feedback Gate!"

# Final verification
log_progress "Final verification..."
if [[ -f "$FEEDBACK_GATE_DIR/feedback_gate_mcp.py" ]] && \
   [[ -f "$CURSOR_MCP_FILE" ]] && \
   [[ -d "$FEEDBACK_GATE_DIR/venv" ]]; then
